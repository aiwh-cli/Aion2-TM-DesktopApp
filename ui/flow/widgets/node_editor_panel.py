...
class NodeEditorPanel(QFrame):
    ...
    # ── Character Items ───────────────────────────────────────────────────────
    # No longer shown/editable here (User-Wunsch, 2026-09-09: "Liste
    # bearbeiten" removed, since the Tasks/Shopping toolbar's "Templates /
    # Standard Templates" tabs now cover assigning things to a character;
    # the leftover read-only "Einkaufsliste ... Keine Einträge" summary
    # this section used to show was removed right after for the same
    # reason). Kept as plain data plumbing only -- FlowController's
    # save_selected_node() round-trips node.character_items through
    # get_character_items()/load_character_items() on every node
    # select/save, and Full View Import still reads/writes character_items
